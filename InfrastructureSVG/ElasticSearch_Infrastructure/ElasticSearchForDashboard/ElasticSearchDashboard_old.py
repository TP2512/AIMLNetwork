...
class SendDataForDashboard:
    ...
    def fill_gnbs_docs(self, list_of_docs, index_name, feature_global_information_doc, time_number, gnbs_data):
    ...
    def fill_ues_docs(self, list_of_docs, index_name, feature_global_information_doc, time_number, gnb_data_list):
        for gnb_data in gnb_data_list:
            ue_results_key_list = list(gnb_data['UE_RESULTS'].keys())
            for index, (ue_name, ue_statistics_data) in enumerate(gnb_data['UE_STATISTICS'].items(), start=0):
                if len(ue_results_key_list) > index:
                    ue_results_data = gnb_data['UE_RESULTS'][ue_results_key_list[index]]
                else:
                    ue_results_data = None
    ...
    def process(self, elk_client, index_name, global_information_body, list_of_data, run_time, automation_manual='Automation') -> None:
        ...
        self.logger.info('Start "fill_gnbs_docs" and "fill_ues_docs" functions')  # -------------------------------
