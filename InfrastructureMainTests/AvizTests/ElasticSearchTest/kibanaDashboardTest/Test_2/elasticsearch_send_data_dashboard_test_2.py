...
from InfrastructureMainTests.AvizTests.ElasticSearchTest.kibanaDashboardTest.Test_1.data_to_elasticsearch import gnb_type, version_list, slave_name_list, ems_version_list, configuration_dict, \
...
def fill_global_information_doc(timestamp, version, index):
    return {
        "timestamp": timestamp,
        ...
        "ACP Version": ems_version_list[index],
    }


def fill_enbs_docs(doc, ues_number, configuration_v):
    pass


def fill_ues_docs(doc, ues_number, configuration_v):
...
def main(elk_client, index_name, ues_number, timestamp) -> None:
    ...
    for index_1, version in enumerate(version_list, start=0):
        ...
        global_information_doc = fill_global_information_doc(timestamp=timestamp, version=version, index=index_1)

        for builder_profile_k, builder_profile_v in features_data_dict.items():
            print(f'-- Builder Profile Name: {builder_profile_k}')
            global_information_doc["builder_name"] = builder_profile_k
            ...
            for configuration_v in configuration_dict:
                ...
                for feature_k, feature_v in builder_profile_v.items():
                    ...
                    for index_3, (feature_scenario_k, feature_scenario_v) in enumerate(feature_v['Status'].items()):
                        ...
                        doc_id = generate_doc_id(index_1, index_3)

                        global_information_doc.update(
                            {
                                "doc_id": f'{doc_id}',
                        ...
                        for time_index in range(5):
                            ...
                            fill_ues_docs(doc=doc, ues_number=ues_number, configuration_v=configuration_v)
                    print()
            print()
        if list_of_docs:
