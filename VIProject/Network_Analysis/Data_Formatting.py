...
so_mapped = pd.merge(links, nssid_info, left_on='so_nssid', right_on='NSSID', how='inner')
so_mapped['source'] = so_mapped.source + '_GNE'
si_mapped = pd.merge(links, nssid_info, left_on='si_nssid', right_on='NSSID', how='inner')
...
dt = pd.DataFrame([])
dt = pd.concat([final["source"], final["target"]], axis=0, ignore_index=True)
dt = dt.drop_duplicates()
node_names = dt.to_list()

# 1.single represents twice in data as GNe and NORMAL
...
# list_of_edges.insert(0, "group no", gr_no)
# list_of_edges.insert(1, "main_graph", main_graph)
# list_of_edges.insert(2, "Network", nw_type)
# list_of_edges.insert(3, "No_of_GNEs", no_of_gne)
# list_of_edges.insert(4, "sub_graph", sub_graph_list)
# list_of_edges.insert(5, "down_link", down_link)
# list_of_edges.insert(6, "dep_nodes", dep_nodes)
#
